...
- **通知渠道**：邮件/钉钉/企微/飞书/通用Webhook推送 + 消息模板(${变量}渲染) + 发送记录 + 测试发送；事件接入公告发布/任务失败/租户到期与配额预警
- **导出中心**：异步导出任务（大数据量不阻塞请求），生成后经当前存储下载
- **登录方式**：密码 + 短信验证码（app.sms开关，mock模式便于联调）+ LDAP/AD（app.ldap）+ OIDC单点登录（app.oidc，Keycloak/Casdoor等）
- **安全增强**：敏感操作二次认证（reauth.enabled开关：删租户/重置密码需携带当前登录密码）、异地登录提醒（站内信）
- **性能**：菜单树/租户品牌Caffeine本地缓存（变更主动失效）
- **开放能力**：Open API（API Key授权范围/限流/过期，HMAC-SHA256签名+时间戳+Nonce防重放，调用明细落库可查，`/api/open/v1` 对外查询用户与公告）、代码生成器（读表结构生成CRUD，预览/下载）
- **可观测性**：监控中心（JVM/系统概览）、健康自检、全局搜索、数据大屏（ECharts）、登录IP归属地（ip2region离线解析）、公告已读回执统计
